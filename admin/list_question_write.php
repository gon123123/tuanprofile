...
if (!empty($result)) {
    ...
    db_connect();
    $sql = "SELECT * FROM question_write LIMIT $index , 17";
                ...
                <?php foreach ($question as $key => $value) : ?>
                    <tr>
                        ...
                        <form action="" method="post">
                            <input type="hidden" name="id_detail" value="<?php echo $value['idWrite']; ?>">
                            <input type="hidden" name="submit" value="detail_question_write">
                            <td><input class="btn btn-sm btn-primary button_submit" type="submit" value="Detail"></td>
                        </form>
                        <form action="" method="post">
                            <input type="hidden" name="id_detail" value="<?php echo $value['idWrite']; ?>">
                            <input class="btn btn-sm btn-warning button_submit" type="hidden" name="submit" value="del_question_write">
...
<ul class="pagination">
    <?php for ($i = 1; $i <= $number_page; $i++) : ?>
        <form action="" method="GET">
            ...
            <input type="hidden" name="submit" value="page_question_write">
